...
- 发布详情统一转换设备和还原任务状态、阶段文案；还原详情突出展示还原范围、来源版本、当前版本和当前阶段
- 普通发布提交后每 3 秒查询一次发布记录，最多等待 1 分钟；超时仅提示仍在发布中，不将未完成任务误判为失败
- 还原成功提示改为直接展示“版本已发布成功”，不再展示“还原任务已创建”
...
- 发布版本校验改用自动清理的临时目录，不再遗留 `tmp/release-validate`
